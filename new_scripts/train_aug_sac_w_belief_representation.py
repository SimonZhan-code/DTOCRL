...
class Trainer():
    ...
    def train(self):
        ...
        for self.global_step in trange(1, self.config['total_step']+1):
            ...
            self.replay_buffer.store(obs, latent, action, reward, next_obs, next_latent, done)
...
if __name__ == "__main__":
    configs = {
        ...
        "dynamic_type": ['gru', 'trans'],
    }
    configs = get_configs(configs)
    for config in configs:
        
        config['exp_tag'] = f"new_logs_online/aug_sac_w_br/{config['dynamic_type']}/{config['env_name']}/{config['delay']}/SEED_{config['seed']}"
