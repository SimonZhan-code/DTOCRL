    ...
    def train(self):
        ...
        for self.global_step in trange(1, self.config['total_step']+1):
            ...
            backward_seq = {
                ...
                'n_return': [torch.FloatTensor(np.array(reward)).unsqueeze(-1).to('cpu')], 
                'n_gamma': [torch.FloatTensor(np.array(self.config['gamma'])).unsqueeze(-1).to('cpu')],
                'next_obs': [torch.FloatTensor(next_obs).to('cpu')], 
                'next_rec_obs': [next_rec_obs.to('cpu')], 
                'next_latent': [next_latent.to('cpu')], 
                'done': [torch.FloatTensor(np.array(done)).to('cpu')],
            }
            max_n = len(delayed_deque['action'])
            # backward
            if self.config['backward_step'] > 1:
                for n in reversed(range(max_n - 1)):
                    ...
                    backward_seq['obs'].append(torch.FloatTensor(delayed_deque['obs'][n]).to('cpu'))
                    backward_seq['rec_obs'].append(next_rec_obs_seq[0, n, :].to('cpu'))
                    backward_seq['latent'].append(next_latents_seq[0, n, :].to('cpu'))
                    backward_seq['action'].append(torch.FloatTensor(delayed_deque['action'][n]).to('cpu'))
                    # n step
                    # backward_seq['n_return'].append(torch.FloatTensor(np.array(n_return)).unsqueeze(-1).to('cpu'))
                    # backward_seq['n_gamma'].append(torch.FloatTensor(np.array(n_gamma)).unsqueeze(-1).to('cpu'))
                    # backward_seq['next_obs'].append(torch.FloatTensor(next_obs).to('cpu'))
                    # backward_seq['next_rec_obs'].append(next_rec_obs.to('cpu'))
                    # backward_seq['next_latent'].append(next_latent.to('cpu'))
                    # backward_seq['done'].append(torch.FloatTensor(torch.FloatTensor(np.array(done))).to('cpu'))
                    # 1 step
                    # backward_seq['n_return'].append(torch.FloatTensor(np.array(delayed_deque['reward'][n])).unsqueeze(-1).to('cpu'))
                    # backward_seq['n_gamma'].append(torch.FloatTensor(np.array(self.config['gamma'])).unsqueeze(-1).to('cpu'))
                    # backward_seq['next_obs'].append(torch.FloatTensor(delayed_deque['obs'][n+1]).to('cpu'))
                    # backward_seq['next_rec_obs'].append(next_rec_obs_seq[0, n+1, :].to('cpu'))
                    # backward_seq['next_latent'].append(next_latents_seq[0, n+1, :].to('cpu'))
                    # backward_seq['done'].append(torch.FloatTensor(torch.FloatTensor(np.array(delayed_deque['done'][n]))).to('cpu'))
                    if len(backward_seq['obs']) == self.config['backward_step']:
                        break
            while len(backward_seq['obs']) < self.config['backward_step']:
                for key in backward_seq:
                    backward_seq[key].append(backward_seq[key][0])
            assert len(backward_seq['obs']) == self.config['backward_step']
            for key in backward_seq:
                    # print(key)
                    backward_seq[key] = torch.stack(backward_seq[key])
                    # print(key, backward_seq[key].shape)
            self.replay_buffer.store(
                obs=backward_seq['obs'], 
                rec_obs=backward_seq['rec_obs'], 
                latent=backward_seq['latent'], 
                action=backward_seq['action'], 
                n_return=backward_seq['n_return'], 
                n_gamma=backward_seq['n_gamma'], 
                next_obs=backward_seq['next_obs'], 
                next_rec_obs=backward_seq['next_rec_obs'], 
                next_latent=backward_seq['next_latent'], 
                done=backward_seq['done'].unsqueeze(-1), 
            )
            ...
            if self.global_step > self.config['learn_start']:
                ...
                # reverse
                # with torch.no_grad():
                #     target_1_val = self.target_1(b_obs, b_action)
                #     target_2_val = self.target_2(b_obs, b_action)
                #     target_val = torch.min(target_1_val, target_2_val)
                #     target_val = (target_val - b_n_return) / b_n_gamma
                #     # target_val = target_val.max(1)[0].unsqueeze(1).repeat(1, self.config['backward_step'], 1)   
                #     # print(target_val.shape)
                #     p_next_action, p_next_log_pi, _ = self.actor.get_action(b_next_rec_obs)
                # critic_1_next_val = self.critic_1(b_next_obs, p_next_action) - self.alpha * p_next_log_pi
                # critic_2_next_val = self.critic_2(b_next_obs, p_next_action) - self.alpha * p_next_log_pi

                # loss_critic_1 = (F.mse_loss(critic_1_next_val, target_val, reduction='none') * (1 - b_done)).mean()
                # loss_critic_2 = (F.mse_loss(critic_2_next_val, target_val, reduction='none') * (1 - b_done)).mean()
...
if __name__ == "__main__":
    configs = {
        ...
        "delay": [int(32), int(128)],
        "dynamic_type": ['trans'],
        "backward_step": [int(2), int(4), int(8)],
        
    }
    configs = get_configs(configs)
    for config in configs:
        # config['batch_size'] = int(config['batch_size'] / config['backward_step'])
        # print(config['batch_size'])
        config['exp_tag'] = f"new_logs_online/new_backward_{config['backward_step']}_reverse"
