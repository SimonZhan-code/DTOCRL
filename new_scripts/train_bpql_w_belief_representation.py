...
class Trainer():
    def __init__(self, config):
        ...
        # checkpoint = torch.load(f"new_checkpoints/trans_reward/{config['env_name']}_trans_Delay_128.pth", map_location=torch.device('cpu'))
        ...
        if config['dynamic_type'] == 'mlp':
            ...
            self.latent_dynamic = TRANS_Dynamic_v1(latent_dim=config['latent_dim'], 
                                         condition_dim=action_dim, 
        ...
        self.critic_1 = Critic(latent_dim=config['latent_dim'], action_dim=action_dim).to(config['device'])
        self.target_1 = Critic(latent_dim=config['latent_dim'], action_dim=action_dim).to(config['device'])
        ...
        self.critic_2 = Critic(latent_dim=config['latent_dim'], action_dim=action_dim).to(config['device'])
        self.target_2 = Critic(latent_dim=config['latent_dim'], action_dim=action_dim).to(config['device'])
        ...
        for self.global_step in trange(1, self.config['total_step']+1):
            ...
            with torch.no_grad():
                ...
                if self.config['dynamic_type'] == 'mlp':
                    # mlp
                    next_latent = self.get_next_latent_mlp_dy(next_latent, list(delayed_deque['action'])[random_delay:])
                elif self.config['dynamic_type'] == 'gru':
                    # gru
                    next_latent = self.get_next_latent_gru_dy(next_latent, list(delayed_deque['action'])[random_delay:])
                elif self.config['dynamic_type'] == 'trans':
                    # trans
                    next_latent = self.get_next_latent_trans_dy(next_latent, list(delayed_deque['action'])[random_delay:])
                    # next_latent = self.get_next_latent_trans_dy_128(next_latent, list(delayed_deque['action'])[random_delay:])
                else:
                    raise NotImplementedError
            ...
            if self.global_step > self.config['learn_start']:
                ...
                with torch.no_grad():
                    ...
                    target_1_next_val = self.target_1(b_next_latent, p_next_action)
                    target_2_next_val = self.target_2(b_next_latent, p_next_action)
    ...
    def get_next_latent_trans_dy_128(self, next_latent, actions):
        delayed_idx = len(actions) - 1
        next_latent = next_latent.unsqueeze(0)
        # timesteps = torch.arange(0, len(actions), dtype=torch.int32).to(self.config['device'])
        # timesteps = torch.arange(0, self.config['delay'], dtype=torch.int32).to(self.config['device'])
        ...
        # pad_rewards = torch.zeros((1, self.config['delay'] - len(actions))).to(self.config['device'])
        # pad_rewards = torch.zeros((1, 128 - len(actions))).to(self.config['device'])
        next_latent = self.latent_dynamic(
            latents=next_latent, 
            actions=torch.concat((torch.FloatTensor(np.array(list(actions))).unsqueeze(0).to(self.config['device']), pad_actions), dim=1),
            # rewards=torch.concat((torch.FloatTensor(np.array(list(rewards))).unsqueeze(0).to(self.config['device']), pad_rewards), dim=1).unsqueeze(-1),
...
if __name__ == "__main__":
    ...
    for config in configs:
        if config['stochastic']:
            config['exp_tag'] = f"baseline_logs_stochastic/bpql_w_br/{config['dynamic_type']}/{config['env_name']}/{config['delay']}/SEED_{config['seed']}"
        else:
            config['exp_tag'] = f"baseline_logs_deterministic/bpql_w_br/{config['dynamic_type']}/{config['env_name']}/{config['delay']}/SEED_{config['seed']}"
