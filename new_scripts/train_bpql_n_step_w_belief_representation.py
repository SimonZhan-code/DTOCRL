        ...
        for self.global_step in trange(1, self.config['total_step']+1):
            ...
            with torch.no_grad():
                ...
                if self.config['dynamic_type'] == 'mlp':
                    ...
                    # trans
                    next_latent, latents_seq = self.get_next_latent_trans_dy(next_latent, list(delayed_deque['action'])[random_delay:], list(delayed_deque['reward'])[random_delay:])
                else:
                    raise NotImplementedError
        ...
        while len(self.log_metric['eval_r']) < 10:
            ...
            with torch.no_grad():
                ...
                if self.config['dynamic_type'] == 'mlp':
                    ...
                    # trans
                    next_latent, _ = self.get_next_latent_trans_dy(next_latent, list(delayed_deque['action'])[random_delay:], list(delayed_deque['reward'])[random_delay:])
                else:
                    raise NotImplementedError
                next_rec_obs = self.auto_encoder.decode(next_latent)
...
if __name__ == "__main__":
    configs = {
        ...
        "dynamic_type": ['trans'],
        "n_steps": [int(1), int(2), int(4), int(8)],
        "stochastic": [True],
    }
    ...
    for config in configs:
        ...
        if config['stochastic']:
            config['exp_tag'] = f"new_logs_online/dbt_bpql_{config['n_steps']}_step_w_br/{config['dynamic_type']}/{config['env_name']}/stochastic_{config['delay']}/SEED_{config['seed']}"
        else:
            config['exp_tag'] = f"new_logs_online/forward_generate_{config['n_steps']}_step_w_br/{config['dynamic_type']}/{config['env_name']}/{config['delay']}/SEED_{config['seed']}"
