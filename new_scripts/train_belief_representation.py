...
class Trainer():
    ...
    def train_latent_dynamic(self):
        ...
        self.optimizer = optim.Adam(self.dynamic.parameters(), lr=self.config['lr'])
        ...
        for self.global_step in trange(100):
            ...
            torch.save({
                ...
                f"new_checkpoints/latent_dynamic/{self.config['env_name']}_{self.config['dynamic_type']}_Delay_{self.config['delay']}.pth")
...
if __name__ == "__main__":
    ...
    for config in configs:
        ...
        config['exp_tag'] = f"new_logs/{config['dynamic_type']}/{config['env_name']}/{config['delay']}"
