...
class MuJoCoRenderer:
    ...
    def renders(self, samples, partial=False, **kwargs):
        ...
        return composite
    def add_text_2_img(self, img, text):
        import cv2
        ...
        color = (0, 0, 0)
        thickness = 2
        img_with_text = cv2.putText(img, text, org, font, font_scale, color, thickness)
    ...
    def composite(self, 
        ...
        for i, path in enumerate(paths):
            ...
            if texts is not None:
                img = self.add_text_2_img(img, texts[i])
            images.append(img)
        if error_mean is not None and error_std is not None:
            import matplotlib.pyplot as plt
            plt.figure(figsize=(10.24, 2.56))
            plt.plot(error_mean.squeeze())
            plt.fill_between(np.arange(error_mean.shape[1]), 
                             (error_mean - error_std).squeeze(), 
                             (error_mean + error_std).squeeze(), 
                             alpha=0.5,)

            fig = plt.gcf()
            fig.canvas.draw()
            error_image = np.array(fig.canvas.renderer.buffer_rgba())[:, :, :3]
...
if __name__ == '__main__':
    ...
    renderer = MuJoCoRenderer(env='halfcheetah')